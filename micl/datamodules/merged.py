...
from ..dataset.vindr import VinDr, LABELS as VINDR_LABELS
from .ms_cxr import MSCXRDataset
...
class MergedDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.ms_cxr_dir = ms_cxr_dir
        self.mimic_cxr_jpg_dir = mimic_cxr_jpg_dir
        self.mscxr_weight = mscxr_weight

    def prepare_data(self) -> None:
        pass

    def setup(self, stage: Optional[str] = None) -> None:
        offset = len(VINDR_LABELS)
        self.train_dataset = ConcatDataset([
            VinDr(train=True, augmentations=self.augmentations),
            MSCXRDataset(self.mimic_cxr_jpg_dir, self.ms_cxr_dir, split='train', labels_offset=offset, multiplier=self.mscxr_weight),
            MSCXRDataset(self.mimic_cxr_jpg_dir, self.ms_cxr_dir, split='val', labels_offset=offset, multiplier=self.mscxr_weight),
        ])
        self.val_datasets = [
            VinDr(train=False), 
            MSCXRDataset(self.mimic_cxr_jpg_dir, self.ms_cxr_dir, split='test', labels_offset=offset),
        ]
